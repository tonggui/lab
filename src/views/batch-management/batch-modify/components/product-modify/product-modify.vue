<template>
  <component :is="container" label="选择要修改的商品" :index="index + 1" class="batch-product-modify">
    <SlideUp group>
      <ProductModifyForm v-for="(item, index) in list" :key="item.id" :deleteabled="deleteabled"  :value="item" :index="index" @change="handleChange" :context="context" @delete="handleDelete" ref="form" />
    </SlideUp>
    <div class="footer">
