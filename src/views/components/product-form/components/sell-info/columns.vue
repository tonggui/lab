    ...
    computed: {
      ...
      stockCol () {
        ...
        return {
          ...
          render: (h, { row }) => <InputNumber placeholder='请输入' precision={0} max={PRODUCT_MAX_STOCK} min={-1} disabled={ this.disabled || disabled || isDisabled(row, this.disabledExistSkuColumnMap, 'stock') || !row.enableStockEditing} />
        }
      },
