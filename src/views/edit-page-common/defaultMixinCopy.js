    ...
    methods: {
      ...
      async getGetNeedAudit (changeOrigin = false) {
        ...
        if (category && category.id) {
          const { poiNeedAudit, categoryNeedAudit } = await fetchGetNeedAudit(
            category.id)
      ...
      //
      //     const { normalAttributes = [], normalAttributesValueMap = {} } = this.getAttributes(
      //       newData)
      //     const { normalAttributesValueMap: oldNormalAttributesValueMap = {} } = this.getAttributes(
      //       oldData)
      ...
      //         if (!isEqual(normalAttributesValueMap[attr.id],
      //           oldNormalAttributesValueMap[attr.id])) {
        ...
        if (this.needAudit) {
          ...
          this.$Modal.confirm({
            ...
            onCancel: () => {
              ...
              this.$router.replace(
                { name: 'productAuditList', query: { wmPoiId: poiId } })
            }
          })
        } else {
          this.handleCancel() // 返回
