...
import validate, { weightOverflow } from '@/views/components/product-form/validate'
...
import computeAuditFieldTips from '@/views/components/product-form/components/audit-field-tip/audit-field-rule'
      ...
      rules: {
        result: {
          'options.changes' () {
            ...
            if (hasSellAttr) {
              return {
                basicInfoList: changes.filter(item => item.field !== 'SPEC'),
                categoryAttrInfoList: []
              }
            }
            return {
              basicInfoList: changes,
              categoryAttrInfoList: []
            }
          },
          'options.product' () {
          ...
          'options.value' () {
            // TODO 依赖其他某一个props怎么办？
            const changes = this.getContext('changes') || []
            const categoryAttrList = this.getData('categoryAttrList') || []
            const hasSellAttr = categoryAttrList.some(v => v.attrType === ATTR_TYPE.SELL)
            // 如果有销售属性，则过滤掉规格
            if (hasSellAttr) {
              return !!changes.filter(item => item.field !== 'SPEC').length
            }
          ...
          events: {
            ...
            change (name) {
              ...
              if (allowSuggestCategory && name) {
                ...
                // 根据商品标题获取推荐类目 (shangou/category/r/suggestCategoryByName)
